...
- **Only `transfer` writes `ledger_entries`/`transactions.status`/`cached_balance`.** Enforced by module discipline *and* by DB permission (`REVOKE UPDATE, DELETE` on `ledger_entries`).
